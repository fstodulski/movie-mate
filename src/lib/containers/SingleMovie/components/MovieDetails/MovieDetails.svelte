<script lang="ts">
  ...
  import { toMovieDetailsUtils } from '$lib/containers/SingleMovie/components/MovieDetails/to-movie-details.utils';
...
<section class="px-4 flex flex-col pt-8 gap-4">
  ...
  {#await data then { credits, movie }}
    {@const movieData = toMovieDetailsUtils({ ...movie, ...credits })}
